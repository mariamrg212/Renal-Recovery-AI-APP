...
st.set_page_config(page_title="Renal Recovery Predictor", layout="centered")
...
# 1. DM Duration (Continuous)
input_features['DM duration'] = st.sidebar.slider(
    'DM duration (Years)',
    float(data_df['DM duration'].min()),
    float(data_df['DM duration'].max()),
    float(data_df['DM duration'].median()),
    step=0.1
)
...
# 8. Hb level (Continuous)
input_features['Hb level (Admission)'] = st.sidebar.slider(
    'Hb level (Admission)',
    float(data_df['Hb level (Admission)'].min()),
    float(data_df['Hb level (Admission)'].max()),
    float(data_df['Hb level (Admission)'].median()),
    step=0.1
)

# 9. NGAL (Continuous)
input_features['NGAL (Admission)'] = st.sidebar.slider(
    'NGAL (Admission)',
    float(data_df['NGAL (Admission)'].min()),
    float(data_df['NGAL (Admission)'].max()),
    float(data_df['NGAL (Admission)'].median()),
    step=1.0
)

# 10. Blood pH (Continuous)
input_features['Blood pH'] = st.sidebar.slider(
    'Blood pH (Admission)',
    float(data_df['Blood pH'].min()),
    float(data_df['Blood pH'].max()),
    float(data_df['Blood pH'].median()),
    step=0.01,
    format='%.2f'
)

# 11. Serum creatinine at discharge (Continuous)
input_features['Serum creatinine at discharge'] = st.sidebar.slider(
    'Serum creatinine at discharge (mg/dL)',
    float(data_df['Serum creatinine at discharge'].min()),
    float(data_df['Serum creatinine at discharge'].max()),
    float(data_df['Serum creatinine at discharge'].median()),
    step=0.1
)
...
# 13. HbA1c (Continuous)
input_features['HbA1c'] = st.sidebar.slider(
    'HbA1c',
    float(data_df['HbA1c'].min()),
    float(data_df['HbA1c'].max()),
    float(data_df['HbA1c'].median()),
    step=0.1
)

# 14. Sofa score (Continuous)
input_features['Sofa score'] = st.sidebar.slider(
    'SOFA score',
    int(data_df['Sofa score'].min()),
    int(data_df['Sofa score'].max()),
    int(data_df['Sofa score'].median())
)

# 15. SAPS II score (Continuous)
input_features['SAPS II score'] = st.sidebar.slider(
    'SAPS II score',
    int(data_df['SAPS II score'].min()),
    int(data_df['SAPS II score'].max()),
    int(data_df['SAPS II score'].median())
)
